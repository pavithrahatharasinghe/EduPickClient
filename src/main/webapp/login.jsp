    ...
    document.getElementById("loginForm").onsubmit = async function(event) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Login failed. Server responded with: ${errorText}`);
            }

            const user = await response.json();
            alert("Login successful");

            // Assuming user status and roles are part of the response for redirection logic
            // Redirect based on the role; adjust as needed
            if (user.role === "admin") {
